...
def lqr(A, B, m, Q, R, M, q, r, b, T):
    ...
    y = (q.T-0.5*r.T@R_inv).T
    p = (b-0.25*r.T@R_inv@r).reshape(1,)
    
    K = -0.5*R_inv@M.T
    k = (-0.5*R_inv@r).reshape(1,)
    out.insert(0,(K,k))
    ...
    for t in range(T-1):
        ...
        P = C + K.T@D@K + E@K
        y = (f.T + 2*k.T@D@K + k.T@E.T + g.T@K).T
        p = (k.T@D@k + g.T@k + h).reshape(1,)
